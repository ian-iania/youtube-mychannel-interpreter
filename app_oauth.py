#!/usr/bin/env python3
"""
Streamlit App - YouTube Playlist Video Search & Transcript (OAuth Version)
Interface para buscar vídeos em TODAS as playlists (públicas + privadas)
USA AUTENTICAÇÃO OAUTH 2.0 para acessar playlists privadas
"""
...
st.set_page_config(
    page_title="YouTube Playlist Manager (OAuth)",
    page_icon="🔐",
    layout="wide",
...
def main():
    ...
    st.markdown("""
        <div style="background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); 
                    ...
                    margin-bottom: 2rem;
                    text-align: center;">
            <h1 style="color: white; margin: 0;">🔐 YouTube Playlist Manager (OAuth)</h1>
            <p style="color: #f0f0f0; margin: 0.5rem 0 0 0;">
                Acesso completo a TODAS as playlists (públicas + privadas + não listadas)
            </p>
        </div>
    """, unsafe_allow_html=True)
    
    st.markdown("### 🔍 Busque, marque e transcreva vídeos de todas as suas playlists")
    
    # Carregar playlists
    playlists = load_playlists()
    
    if not playlists:
        st.error("❌ Nenhuma playlist encontrada no diretório 'playlists_oauth/'")
        st.info("Execute o script export_playlists_oauth.py primeiro para exportar suas playlists com OAuth.")
        st.code("python scripts/export_playlists_oauth.py", language="bash")
        return
    
    st.success(f"✅ {len(playlists)} playlists carregadas (públicas + privadas)")
